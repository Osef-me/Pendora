...
impl BeatmapWorker {
    pub async fn start(&self) -> Result<(), BeatmapWorkerError> {
        ...
        loop {
            ...
            let pending_beatmaps = PendingBeatmapRow::last_pending_beatmap(&pool).await.map_err(|e| BeatmapWorkerError::DatabaseError(e.to_string()))?;
            ...
            tracing::info!("Processing pending beatmap: id={}, hash={}", pending_beatmap.id, pending_beatmap.osu_hash);
            PendingBeatmapRow::delete_by_id(&pool, pending_beatmap.id).await.unwrap();
            
            tracing::debug!("Fetching beatmap from osu! API for hash: {}", pending_beatmap.osu_hash);
            let beatmap = self
                .osu_api_service
            ...
            tracing::debug!("Beatmap fetched: osu_id={}, mode={}, cs={}", beatmap.map_id, beatmap.mode, beatmap.cs);
            if !is_allowed_beatmap(beatmap.mode, beatmap.cs).await {
                tracing::warn!("Beatmap not allowed: mode={}, cs={}", beatmap.mode, beatmap.cs);
                continue;
            }

            let beatmapset = beatmap.mapset.clone().unwrap();
            tracing::debug!("Beatmapset: id={}, artist={}, title={}", beatmapset.mapset_id, beatmapset.artist, beatmapset.title);
            ...
            tracing::info!("Processing beatmap: osu_id={}, difficulty={}", beatmap_row.osu_id.unwrap(), beatmap_row.difficulty);
            process_beatmap(&beatmap, &calc, osu_path, &mut beatmap_row)
                .await
                .unwrap();

            beatmapset_row.beatmaps.push(beatmap_row);
            tracing::info!("Inserting beatmapset into database: osu_id={}", beatmapset_row.osu_id.unwrap());
            insert_full_beatmapset(&self, &beatmapset_row).await.unwrap();
            tracing::info!("Successfully processed and inserted beatmapset: osu_id={}", beatmapset_row.osu_id.unwrap());
        }
    }
}
