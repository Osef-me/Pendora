...
pub fn determine_main_pattern(skillset_scores: &Ssr, beatmap: &Beatmap) -> serde_json::Value {
    ...
    let ln_ratio = if total_objects > 0 { ln_count as f64 / total_objects as f64 } else { 0.0 };
    
    debug!("LN count: {}, total objects: {}, LN ratio: {:.3}", ln_count, total_objects, ln_ratio);
    ...
    pattern_parts.extend(top_two.iter().map(|s| serde_json::Value::String(s.to_string())));
