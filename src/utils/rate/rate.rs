...
pub async fn bulk_rate(
    ...
    let rates_and_hashes: Vec<(f64, String)> = valid_rates
        .par_iter()
        .map(|&rate_value| {
            process_single_rate(rate_value, &maps, beatmap_id)
        })
        .collect();

    info!(
        "bulk_rate completed for beatmap_id: {}, generated {} variations",
        beatmap_id, rates_and_hashes.len()
    );
...
fn process_single_rate(
    ...
    let _file_path = FileManager::save_compressed_file(beatmap_id, &hash, &compression_result.compressed_data).unwrap();
